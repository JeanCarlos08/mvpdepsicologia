...
# Carrega um .env que esteja no mesmo diretório deste arquivo, independente do CWD.
...
try:
	if _LOCAL_ENV_PATH.exists():
		...
		# fallback para comportamento padrão (procura em CWD e pais)
		load_dotenv()
except Exception:
	# Não bloquear inicialização se houver falha ao ler .env
	pass
...
def _load_db_config() -> Dict[str, str]:
	...
	if secrets is not None:
		try:
			...
			# Ignora falha ao ler st.secrets quando não existe secrets.toml
			config = None
	if config is None:
...
def get_connection():
	"""Retorna uma nova conexão (PostgreSQL ou SQLite fallback)."""
	...
	try:
		...
		print(f"⚠️ Erro PostgreSQL ({e}), usando SQLite fallback")
		_USE_SQLITE_FALLBACK = True
		return sqlite3.connect(str(_SQLITE_DB))
...
def _get_cursor(conn):
	"""Retorna cursor compatível (dict-like para PostgreSQL, row_factory para SQLite)."""
